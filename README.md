...
- **Выборы по округам** — 27 округов игровой карты на 124 мандата. Голоса не
  вводятся, а разыгрываются: каждой партии в округе выпадает 1–10 — не
  равновероятно, а по треугольному распределению (среднее двух обычных
  бросков), так что разгромные исходы редки, как и в жизни, — и к броску
  прибавляются модификаторы, а эти числа делятся между партиями
  пропорционально. Модификаторы — средняя поддержка по населённым пунктам,
  свободный модификатор на партию в конкретном округе и «настроение по
  стране» — тот же модификатор, но один сразу на все округа партии, как волна
  за или против неё по всей карте. Любое число, отрицательное работает как
  штраф; причина не привязана к чему-то одному — дебаты, скандал, что угодно
  по ходу партии. Кнопка «Убрать все модификаторы» разом чистит все
  выставленные вручную числа, местные и общий. Места округа делятся по методу
...
295 тестов: бизнес-логика (валидация мест, справочник, созывы, удаление созыва,
