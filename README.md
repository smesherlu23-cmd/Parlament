...
- **Коалиции** — партии можно свести в блок, который голосует заодно. Своих
  ...
  весом и перевесом над большинством, а строка внизу считает большинство по
...
- **Голоса и места** — в итогах выборов у каждой партии рядом с мандатами
  стоит доля голосов по стране: округа сводятся со своими весами по числу
  мандатов, потому что округ на десять мест представляет больше людей, чем
  округ на два. Числа расходятся — округа делятся по большинству, и партия
  набирает мест то больше своей доли, то меньше, — ровно за этим на них и
  смотрят рядом. Видно и в легенде, и в правой панели, и в выгруженной
  картинке. У состава, набранного руками, голосов нет вовсе: там за местами
  не стоит никакого голосования, и ноль вместо них был бы неправдой.
...
- **Карта** — округа нарисованы своими границами и красятся в цвет
  победившей партии, поверх — их названия и число мандатов. Кадр обрезан по
  самому архипелагу: он вытянут с запада на восток, и в исходном холсте 16:9
  четверть высоты пустовала. Куда встанет подпись, решается по месту: под
  ней ищется самая широкая горизонталь округа и подбирается кегль, а туда,
  где название не помещается никаким, идёт номер округа — города метрополий
  лежат вплотную и мельче прочих. Цвет буквы выбирается по яркости заливки,
  поэтому подпись читается на любой партийной краске. Клик по округу
...
388 тестов: бизнес-логика (валидация мест, справочник, созывы, удаление созыва,
