...
- **Карта** — округа нарисованы своими границами и красятся в цвет
  победившей партии, поверх — их названия. Только названия: номер округа и
  число мандатов карту засоряли, а читать по ним нечего — номер и так
  подписан на игровой карте, мандаты видны в разборе по клику. Кадр обрезан
  по самому архипелагу: он вытянут с запада на восток, и в исходном холсте
  16:9 четверть высоты пустовала. Куда встанет подпись, решается по месту:
  под ней ищется самая широкая горизонталь округа и подбирается кегль, а где
  название не помещается никаким, округ остаётся без подписи — лучше пусто,
  чем поверх соседа. Цвет буквы выбирается по яркости заливки, поэтому
  подпись читается на любой партийной краске. Клик по округу
  показывает разбор: база, модификаторы (местный, по острову, по стране),
  колебание, итоговый процент и места.
...
392 теста: бизнес-логика (валидация мест, справочник, созывы, удаление созыва,
