...
- **Распределение мест** — по полю ввода на партию; схема, легенда, остаток и
  ...
  палаты, отрицательные числа и буквы не принимаются.
- **Выборы по округам** — 27 округов игровой карты на 147 мест. Голоса
  вносятся таблицей «округ × партия» вручную или загружаются файлом (кнопка
  «Шаблон таблицы» отдаёт заготовку под текущие округа и партии). Места
  округа делятся пропорционально голосам, победитель забирает большинство;
  если борьбы не было — весь округ.
- **Карта** — округа отмечены маркерами и красятся в цвет победившей партии,
  клик по маркеру показывает расклад округа. Отсюда же открываются выборы.
  Карту тоже можно выгрузить в PNG. Проект, начатый до появления карты,
  округов не получает молча — их предлагается добавить кнопкой на этом
  экране, потому что вместе с ними меняется размер палаты.
...
163 теста: бизнес-логика (валидация мест, справочник, созывы, удаление созыва,
выборы по округам, разбор таблицы результатов, свои цвета, файл проекта),
...
parlament/
  model.py                  партия, созыв, округ, проект; сериализация в JSON
  elections.py              деление мест округа по голосам
  district_seed.py          округа игровой карты: места и точки маркеров
  votes_import.py           разбор таблицы с результатами выборов
  ...
  ui/
    ...
    map_view.py             экран карты: маркеры округов и сводка
    map_chart.py            холст карты: подложка, маркеры, попадание клика
    map_export.py           отрисовка карты в PNG (Pillow)
    elections_view.py       экран выборов: таблица «округ × партия»
    votes_file.py           чтение таблицы результатов с диска и шаблон
...
assets/map/                 подложка карты (в репозиторий не входит)
packaging/windows/
