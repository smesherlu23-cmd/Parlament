...
class TestRpc(ServiceTestCase):
    def setUp(self):
        super().setUp()
        self.rpc = Rpc(self.service)

    def call(self, method, **params):
        return self.rpc.handle_line(json.dumps({"id": 1, "method": method, "params": params}))

    def test_successful_call_returns_full_state(self):
        response = self.call("party.create", name="Народный союз", color="#0088b0", abbr="НС")
        self.assertTrue(response["ok"])
        self.assertEqual(len(response["result"]["state"]["parties"]), 1)

    def test_validation_error_is_typed(self):
        response = self.call("party.create", name="", color="#0088b0")
        self.assertFalse(response["ok"])
        self.assertEqual(response["error"]["kind"], "validation")

    def test_unknown_method(self):
        response = self.call("party.fly")
        self.assertEqual(response["error"]["kind"], "protocol")

    def test_malformed_line(self):
        response = self.rpc.handle_line("не json")
        self.assertEqual(response["error"]["kind"], "protocol")

    def test_missing_parameter(self):
        response = self.call("party.delete")
        self.assertEqual(response["error"]["kind"], "validation")

    def test_full_game_flow(self):
        # Сценарии 1–4 ТЗ одним прогоном через протокол.
        left = self.call("party.create", name="Народный союз", color="#0088b0")["result"]["partyId"]
        right = self.call("party.create", name="Партия труда", color="#d6006c")["result"]["partyId"]
        conv = self.service.project.active_convocation.id

        self.call("seats.set", convocationId=conv, partyId=left, seats=65)
        self.call("seats.set", convocationId=conv, partyId=right, seats=55)
        fixed = self.call("convocation.fix")

        state = fixed["result"]["state"]
        self.assertEqual(len(state["convocations"]), 2)
        self.assertEqual(state["convocations"][0]["seats"][left], 65)
        self.assertEqual(state["convocations"][1]["seats"], {})
        self.assertEqual(state["activeConvocationId"], state["convocations"][1]["id"])
