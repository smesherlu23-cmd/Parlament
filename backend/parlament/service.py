...
class ParlamentService:
    ...
    def state(self) -> dict:
        ...
        return {
            "totalSeats": project.total_seats,
            "rows": project.rows,
            "parties": [p.to_dict() for p in project.parties],
            "convocations": [c.to_dict() for c in project.convocations],
            "activeConvocationId": project.active_convocation.id,
            "projectPath": str(self.path),
            "projectName": self.path.name,
            "isDefaultProject": self.path == self.default_path,
            "savedToDisk": self._loaded_from_disk,
            "nextConvocationName": self.next_convocation_name(),
        }
