...
def _roll_breakdown(result) -> str:
    """«60,0 (+2 оч.) − 4,0 + 1,2 − 0,8 = 58,4» — из чего сложилась доля партии.

    Местная поправка показана рядом с базой, в скобках и в очках — не
    отдельным слагаемым: она уже входит в базу (та же монета, что и живые
    очки поддержки, см. `PartyResult.base`), и прибавить её ещё раз значило
    бы посчитать дважды. Настроение по стране, сдвиг по острову и колебание
    остаются отдельными слагаемыми в процентных пунктах — они начисляются
    уже поверх базы.
    ...
    if result is None:
        return "—"
    base_part = f"{result.base:.1f}".replace(".", ",")
    if result.modifier:
        sign = "+" if result.modifier > 0 else "−"
        base_part += f" ({sign}{abs(result.modifier):.0f} оч.)"
    parts = [base_part]
    for value in (result.national, result.island):
        if value:
    ...
    # Штрафы (по стране или по острову) могли увести сумму в минус, а доля
    # ниже нуля не бывает: иначе партия вычитала бы голоса у остальных. Без
    # оговорки строка выглядела бы арифметической ошибкой. Местная поправка
    # сюда не приводит — она ограничена нулём ещё на уровне очков, раньше,
    # чем эта сумма вообще считается.
    if raw < 0:
        line += " (не ниже нуля)"
    return line
