...
def compute_seats(
    ...
    # подобран под одну палату на 120 мест — стоило ей вырасти, как соседи по
    # дуге сходились и кружки наезжали друг на друга. Теперь предел берётся по
    # обоим расстояниям, и схема выдерживает любой размер палаты.
