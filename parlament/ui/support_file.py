...
#: Строки-подсказки над заголовком. Начинаются с «#» — разбор их пропускает,
#: так что можно и стереть, и оставить как есть: правит только сами очки.
_HELP_LINES = (
    "# ТАБЛИЦА ПОДДЕРЖКИ. Столбцы: Округ, Населённый пункт, дальше — очки "
    "по одному столбцу на партию. Пустая клетка = 0 очков.",
    "# Для города (копилка одна на весь город) — строка с именем города "
    "в «Округ» и пустым «Населённый пункт», например: Гаффинсвик,,4,,2",
    "# Эти две строки с «#» можно стереть — при загрузке они пропускаются.",
)
...
def export_support_template(project: Project) -> bytes:
    ...
    в столбце «Округ» — «Населённый пункт» у неё пустой, чтобы не выглядеть
    сельским пунктом с тем же именем, что и сам город. Перед заголовком —
    две строки-подсказки с «#» о том, как читать файл; `parse_support_csv`
    их пропускает. Сохраняем с BOM, иначе Excel открывает кириллицу
    кракозябрами.
    """
    buffer = io.StringIO()
