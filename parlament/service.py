...
class ParlamentService:
    ...
    def vote_shares(self, convocation_id: str) -> dict[str, float]:
        ...
        Округа взвешиваются по населению: за долей в округе на сорок тысяч
        человек стоит вчетверо больше избирателей, чем за такой же долей в
        округе на десять, и считать их наравне значило бы приравнять хутор к
        столице.

        Именно по населению, а не по мандатам: мандаты по островам разложены
        неровно. На Вакуле выходит около шести тысяч человек на мандат, на
        Каспиане — около девяти с половиной, и по мандатам голос западного
        избирателя весил бы в полтора раза больше центрального. Расхождение
        мест с голосами — то самое, ради чего эти числа и показывают рядом;
        считать голоса по мандатам значило бы отчасти его и спрятать.

        Считается по всем партиям округа, включая не прошедших барьер: это
        доля голосов, а не мест.
        ...
        for district_id, per_party in conv.results.items():
            weight = self.district_population(district_id)
            if weight <= 0:
                continue
            total += weight
    ...
    def district_population(self, district_id: str) -> float:
        """Сколько человек живёт в округе — по данным игровой карты.

        Берётся из карты, а не из проекта: население — такой же её факт, как
        название округа и число мандатов, и заводить его руками нельзя.
        Иначе добавленный на экране поддержки хутор менял бы вес округа на
        выборах — а он ставится ради очков популярности, а не ради этого.

        Округ, которого на карте нет (проекты, начатые до её появления, —
        у них и номеров не было), считается по мандатам: столько человек,
        сколько в среднем приходится на мандат по стране. Иначе такой округ
        выпал бы из подсчёта голосов вовсе.
        """
        district = self._require_district(district_id)
        people = district_seed.district_population(district.code)
        if people > 0:
            return people
        average = (district_seed.TOTAL_POPULATION
                   / max(1, district_seed.SEED_TOTAL_SEATS))
        return district.seats * average
